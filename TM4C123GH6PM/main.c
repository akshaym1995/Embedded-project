...
float disp=0, thetax=0,thetay=0,thetaz=0;
float xa,ya,za,T,xg,yg,zg;
float intdisp=0, intthetax=0,intthetay=0,intthetaz=0;
float GPS_pos_X=0,GPS_pos_Y=0,GPS_pos_Z=0;
...
void compute_position(float Accx,float Accy,float Accz,float gx,float gy,float gz,float *Disp,float *thetax,float *thetay,float *thetaz)
{
   ...
   // float Dispx,Dispy,Dispz;
    ...
    amplitude=Dispx*Dispx+Dispy*Dispy+Dispz*Dispz;
    *Disp=(double)sqroot (amplitude);
    *thetax+=gx*0.01;
    *thetay+=gy*0.01;
    *thetaz+=gz*0.01;
...
{   unsigned long int Time;
   ...
   // UART_Init();
    ...
    {   NVIC_EN0_R = 0x00000000;        /*  disable interrupt 5 in NVIC */
        ...
        //converting gps lat-long coordinates to cartesian frame
        conv_gps_lat_long(Latitude,Longitude,&GPS_pos_X,&GPS_pos_Y,&GPS_pos_Z);


        //Kalman filter for data fusion to be implemented here

        //Angle turned in X direction
        //sprintf(buffer," thetax = %.2f\r\n",thetax);
        intthetax =(int)(thetax*1000000);
        printf("%d\n",intthetax);

        //Angle turned in Y direction
        intthetay =(int)(thetay*1000000);
        printf("%d\n",intthetay);

        //ANgle turned in Z direction
        intthetaz =(int)(thetaz*1000000);
        printf("%d\n",intthetaz);

        //Displacement
        intdisp =(int)(disp*1000000);
        printf("%d\n",intdisp);
